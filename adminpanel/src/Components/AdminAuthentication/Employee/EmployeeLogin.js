...
import EmployeeAuthService from '../../services/Employee/auth.employee.service';
    ...
    const handleLogin = (event) => {
        ...
        if(values.username && values.password ){
            if(values.password.length >= 8){
                EmployeeAuthService.authenticateRLNEmployee(values.username, values.password)
                .then((response) => {
                    console.log(response);
                    if(response.token){
                        console.log("Login Success");
                        localStorage.setItem("adminType", "employee");
                        Swal.fire({
                            title: 'Success!',
                            text: 'Login Success',
                            icon: 'success',
    ...
    return (
        ...
        <LoginContainer>
            <LoginLeft>
                ...
                <h2 style={{marginBottom:'40px'}}>Employee Admin</h2>
